...
- 不适合树形视图的话题必须保持原始平面话题地址。脚本会读取 `/t/{topicId}.json` 的话题数据，私信话题（`archetype: private_message`）、Banner 话题（`archetype: banner`）、Post Voting / 问答投票话题（`is_post_voting` 或 `subtype: question_answer`）、普通投票帖（首帖或帖子数据里包含 `polls` / `polls_votes` / `has_polls` 等投票字段），以及 Discourse 只给出通用 `topic` 占位 slug、会导致树形路由失效的话题，不会改写到 `/n/...`。
